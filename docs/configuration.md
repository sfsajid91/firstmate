...
For Pi, pi-signed, and OMP secondmate launches, `fm-spawn.sh` starts the selected executable with `-e` pointed at the secondmate home's own tracked primary extensions (`.pi/extensions/` or `.omp/extensions/`), already present from the secondmate home's git worktree.
