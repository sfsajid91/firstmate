...
2. Confirm the OMP primary auto-loaded both project extensions from `.omp/extensions/`; if not, restart with `-e __FM_OMP_TURNEND_EXT__ -e __FM_OMP_EXT__` as a fallback.
3. First cycle only: make the one required `fm_watch_arm_pi` call.
   Use `/fm_watch_arm_pi` only as a human-entered fallback.
...
6. Ordinary same-process session replacement (`/new`, `/resume`, `/fork`, reload) retires only the prior generation; call `fm_watch_arm_pi` once for the first cycle of the replacement session without restarting OMP.
   The generation-owner contract lives in `.omp/extensions/fm-primary-pi-watch.ts` (re-exporting `.pi/extensions/fm-primary-pi-watch.ts`).
...
10. Missing, failed, or unhealthy cycle only: if a later notification explicitly reports one of those repair conditions, drain queued wakes, inspect the failure text, call `fm_watch_arm_pi`, and restart OMP with both extensions loaded if needed.
...
Both are tracked, project-local `.omp/extensions/*.ts` wrappers that OMP auto-discovers natively; `bin/fm-session-start.sh` reports when the running OMP session has not loaded both required extensions.
