...
omp_ext="$FM_ROOT/.omp/extensions/fm-primary-pi-watch.ts"
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    pi|pi-signed)
      ...
      ;;
    omp)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the Pi tool fm_watch_arm_pi, or restart OMP with -e ' "$omp_turnend_ext" ' -e ' "$omp_ext" ' if the extensions are not loaded.'
      ;;
    opencode)
